...
data2 <- data.pca$x[,c(1,2,3,4,5)]
data2 <- cbind(data.labels, data2)
data2 <- as.data.frame(data2)

colnames(data2)[1] <- "Diagnosis"

data2$Diagnosis[data2$Diagnosis == 0] <- "B"
data2$Diagnosis[data2$Diagnosis == 1] <- "M"
...
data.lda1 <- lda(Diagnosis~PC1 + PC2 + PC3 + PC4 + PC5, data=learning.set)
data.qda1 <- qda(Diagnosis~PC1 + PC2 + PC3 + PC4 + PC5, data=learning.set)
data.knn1 <- ipredknn(Diagnosis~PC1 + PC2 + PC3 + PC4 + PC5, data=learning.set, k=5)
data.logit1 <- glm(Diagnosis~PC1 + PC2 + PC3 + PC4 + PC5, family=binomial(link="logit"), data=learning.set)
data.tree1 <- rpart(Diagnosis~PC1 + PC2 + PC3 + PC4 + PC5, data=learning.set)
data.bagging1 <- bagging(Diagnosis~PC1 + PC2 + PC3 + PC4 + PC5, data=learning.set)
data.randomForest1 <- randomForest(Diagnosis~PC1 + PC2 + PC3 + PC4 + PC5, data=learning.set)

data.lda2 <- lda(Diagnosis~PC1 + PC2 + PC3, data=learning.set)
data.qda2 <- qda(Diagnosis~PC1 + PC2 + PC3, data=learning.set)
data.knn2 <- ipredknn(Diagnosis~PC1 + PC2 + PC3, data=learning.set, k=5)
data.logit2 <- glm(Diagnosis~PC1 + PC2 + PC3, data=learning.set, family=binomial(link="logit"))
data.tree2 <- rpart(Diagnosis~PC1 + PC2 + PC3, data=learning.set)
data.bagging2 <- bagging(Diagnosis~PC1 + PC2 + PC3, data=learning.set)
data.randomForest2 <- randomForest(Diagnosis~PC1 + PC2 + PC3, data=learning.set)

data.lda3 <- lda(Diagnosis~PC1 + PC2 + PC5, data=learning.set)
data.qda3 <- qda(Diagnosis~PC1 + PC2 + PC5, data=learning.set)
data.knn3 <- ipredknn(Diagnosis~PC1 + PC2 + PC5, data=learning.set, k=5)
data.logit3 <- glm(Diagnosis~PC1 + PC2 + PC5, data=learning.set, family=binomial(link="logit"))
data.tree3 <- rpart(Diagnosis~PC1 + PC2 + PC5, data=learning.set)
data.bagging3 <- bagging(Diagnosis~PC1 + PC2 + PC5, data=learning.set)
data.randomForest3 <- randomForest(Diagnosis~PC1 + PC2 + PC5, data=learning.set)
...
errors <- matrix(c(error.lda1, error.qda1, error.knn1, error.logit1, error.tree1, error.bagging1, error.randomForest1, error.lda2, error.qda2, error.knn2, error.logit2, error.tree2, error.bagging2, error.randomForest2, error.lda3, error.qda3, error.knn3, error.logit3, error.tree3, error.bagging3, error.randomForest3), nrow=7, ncol=3, dimnames =list(c("error.lda", "error.qda", "error.knn",  "error.logit", "error.tree", "error.bagging", "error.randomForest"), c("1 features subset", "2 features subset","3 features subset")))
errors <- round(errors,5)
print(xtable(errors, type = "latex"), file = "3.tex")
...
sensitivity.lda1 <- sensitivity(conf.mat.lda1)
sensitivity.qda1 <- sensitivity(conf.mat.qda1)
sensitivity.knn1 <- sensitivity(conf.mat.knn1)
sensitivity.logit1 <- sensitivity(conf.mat.logit1)
sensitivity.tree1 <- sensitivity(conf.mat.tree1)
sensitivity.bagging1 <- sensitivity(conf.mat.bagging1)
sensitivity.randomForest1 <- sensitivity(conf.mat.randomForest1)

sensitivity.lda2 <- sensitivity(conf.mat.lda2)
sensitivity.qda2 <- sensitivity(conf.mat.qda2)
sensitivity.knn2 <- sensitivity(conf.mat.knn2)
sensitivity.logit2 <- sensitivity(conf.mat.logit2)
sensitivity.tree2 <- sensitivity(conf.mat.tree2)
sensitivity.bagging2 <- sensitivity(conf.mat.bagging2)
sensitivity.randomForest2 <- sensitivity(conf.mat.randomForest2)

sensitivity.lda3 <- sensitivity(conf.mat.lda3)
sensitivity.qda3 <- sensitivity(conf.mat.qda3)
sensitivity.knn3 <- sensitivity(conf.mat.knn3)
sensitivity.logit3 <- sensitivity(conf.mat.logit3)
sensitivity.tree3 <- sensitivity(conf.mat.tree3)
sensitivity.bagging3 <- sensitivity(conf.mat.bagging3)
sensitivity.randomForest3 <- sensitivity(conf.mat.randomForest3)

sensitivity <- matrix(c(sensitivity.lda1, sensitivity.qda1, sensitivity.knn1, sensitivity.logit1, sensitivity.tree1, sensitivity.bagging1, sensitivity.randomForest1, 
                   sensitivity.lda2, sensitivity.qda2, sensitivity.knn2, sensitivity.logit2, sensitivity.tree2, sensitivity.bagging2, sensitivity.randomForest2, 
                   sensitivity.lda3, sensitivity.qda3, sensitivity.knn3, sensitivity.logit3, sensitivity.tree3, sensitivity.bagging3, sensitivity.randomForest3), 
                 nrow=7, ncol=3, dimnames =list(c("LDA", "QDA", "KNN",  "Logit", "Tree", "Bagging", "Random Forest"), c("1 features subset", "2 features subset","3 features subset")))
sensitivity <- round(sensitivity,5)
print(xtable(sensitivity, type = "latex"), file = "3.tex")

# Specificity


specificity.lda1 <- specificity(conf.mat.lda1)
specificity.qda1 <- specificity(conf.mat.qda1)
specificity.knn1 <- specificity(conf.mat.knn1)
specificity.logit1 <- specificity(conf.mat.logit1)
specificity.tree1 <- specificity(conf.mat.tree1)
specificity.bagging1 <- specificity(conf.mat.bagging1)
specificity.randomForest1 <- specificity(conf.mat.randomForest1)

specificity.lda2 <- specificity(conf.mat.lda2)
specificity.qda2 <- specificity(conf.mat.qda2)
specificity.knn2 <- specificity(conf.mat.knn2)
specificity.logit2 <- specificity(conf.mat.logit2)
specificity.tree2 <- specificity(conf.mat.tree2)
specificity.bagging2 <- specificity(conf.mat.bagging2)
specificity.randomForest2 <- specificity(conf.mat.randomForest2)

specificity.lda3 <- specificity(conf.mat.lda3)
specificity.qda3 <- specificity(conf.mat.qda3)
specificity.knn3 <- specificity(conf.mat.knn3)
specificity.logit3 <- specificity(conf.mat.logit3)
specificity.tree3 <- specificity(conf.mat.tree3)
specificity.bagging3 <- specificity(conf.mat.bagging3)
specificity.randomForest3 <- specificity(conf.mat.randomForest3)

specificity <- matrix(c(specificity.lda1, specificity.qda1, specificity.knn1, specificity.logit1, specificity.tree1, specificity.bagging1, specificity.randomForest1, 
                        specificity.lda2, specificity.qda2, specificity.knn2, specificity.logit2, specificity.tree2, specificity.bagging2, specificity.randomForest2, 
                        specificity.lda3, specificity.qda3, specificity.knn3, specificity.logit3, specificity.tree3, specificity.bagging3, specificity.randomForest3), 
                      nrow=7, ncol=3, dimnames =list(c("LDA", "QDA", "KNN",  "Logit", "Tree", "Bagging", "Random Forest"), c("1 features subset", "2 features subset","3 features subset")))
specificity <- round(specificity,5)
print(xtable(specificity, type = "latex"), file = "3.tex")
